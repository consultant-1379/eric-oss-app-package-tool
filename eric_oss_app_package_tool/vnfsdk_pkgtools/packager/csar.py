...
META_ENTRY_OTHERDEFINITIONS_KEY = 'Entry-OtherDefinitions'
META_ENTRY_IMAGES_KEY = 'Entry-Images'
...
FILES_IMAGES = 'OtherDefinitions/ASD/Images'
...
def write(source, entry, destination, args):
    logging.info('csar.py: writing to file')
    source = os.path.expanduser(source)
    destination = os.path.expanduser(destination)


    if os.path.exists( '/target/'+entry ):
        try:
            with open('/target/'+entry) as info:
                metadata = yaml.load(info)
        except:
            print("failed to load /target/"+entry)
    else:
        metadata = BASE_METADATA.copy()

    metadata[META_CREATED_BY_KEY] = args.created_by
    ...
    image_directory = os.path.join(source, FILES_IMAGES)
    for file in os.listdir(image_directory):
        if file.endswith(".tar"):
            image_path = os.path.join(image_directory, file)
            metadata[META_ENTRY_IMAGES_KEY] = os.path.relpath(image_path, source)

    LOG.debug('Compressing root directory to ZIP')
    with zipfile.ZipFile("../" + destination, 'w', zipfile.ZIP_DEFLATED, allowZip64=True) as f:
        if args.digest:
            metadata_hash = utils.calc_string_hash(yaml.dump(metadata, default_flow_style=False), args.digest.upper())
            manifest_file.digests[META_FILE] = (args.digest, metadata_hash)
            ...
            for file in files:
                if file != 'Tosca.meta':
                    ...
                    if file_full_path!=manifest_file_full_path:
                        ...
                        LOG.debug('Writing file to archive: {0}'.format(file_relative_path))
                    ...
                    logging.info('Skipping Tosca.Meta')
            # add empty dir
            for dir in dirs:
                ...
                if len(os.listdir(dir_full_path)) == 0:
                    ...
                    LOG.debug('Writing dir to archive: {0}'.format(dir_relative_path))
        ...
        if manifest_file:
            ...
            if args.certificate and args.privkey:
                ...
                # write cms into it
                manifest_file_full_path = manifest_file.update_to_file(True)
            LOG.debug('Writing manifest to archive: {0}'.format(args.manifest))
...
class _CSARReader(object):
    ...
    @property
    def entry_otherdefinitions(self):
        return self.metadata.get(META_ENTRY_OTHERDEFINITIONS_KEY)
    ...
    def _validate(self, no_verify_cert):
        ...
        validate_key(META_ENTRY_OTHERDEFINITIONS_KEY)
